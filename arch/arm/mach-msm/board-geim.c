...
#ifdef CONFIG_KEYBOARD_STMPE1801 //moon
...
#ifdef CONFIG_KEYBOARD_STMPE_IRQ
#include <linux/input/matrix_keypad.h>
#include <linux/input.h>
#include <linux/input/stmpe1801.h>
#endif

#define _CONFIG_MACH_GEIM // Temporary flag
...
static const struct matrix_keymap_data cust_keymap_data = {
	.keymap	= cust_keymap,
...
#if 0 // temporarily block for Keypad test
...
static struct marimba_fm_platform_data marimba_fm_pdata = {
	...
	//temporarily block for keypad test .irq = MSM_GPIO_TO_INT(FM_GPIO),
	.vreg_s2 = NULL,
	.vreg_xo_out = NULL,
...
#ifdef CONFIG_KEYBOARD_STMPE_IRQ
static struct i2c_board_info stmpe1801_devices[] = {
{
...
static void geim_usb_power(int onoff, char *path)
{
#if 0
...
void geim_chg_connected(enum chg_type chgtype)
{
...
static void geim_usb_cb(u8 attached, struct fsausb_ops *ops)
{
...
static void geim_charger_cb(u8 attached, struct fsausb_ops *ops)
{
...
static void geim_jig_cb(u8 attached, struct fsausb_ops *ops)
{
...
static void geim_fsa880_reset_cb(void)
{
	pr_info(" [BATT] Board file [FSA880]: Reset Callback\n");
}

/* For uUSB Switch */
static struct fsausb_platform_data geim_fsa880_pdata = {
       .intb_gpio      = MSM_GPIO_TO_INT(GPIO_MUSB_INT),
       .usb_cb         = geim_usb_cb,
       .uart_cb        = NULL,
       .charger_cb     = geim_charger_cb,
       .jig_cb         = geim_jig_cb,
	.ovp_cb		= jena_ovp_cb,
       .reset_cb       = geim_fsa880_reset_cb,
};
...
static struct i2c_board_info fsa880_i2c_devices[] = {
	{
		...
		.platform_data =  &geim_fsa880_pdata,
...
#if defined(CONFIG_KEYBOARD_STMPE1801) || defined(CONFIG_KEYBOARD_STMPE_IRQ)
	i2c_register_board_info(MSM_GSBI1_QUP_I2C_BUS_ID,
				stmpe1801_devices,
				ARRAY_SIZE(stmpe1801_devices));
#endif

}

static struct msm_gpio qup_i2c_gpios_io[] = { // yjh
...
#endif /* defined(_CONFIG_MACH_GEIM) || ... */
...
static void lcdc_geim_gpio_init(void)
{
	int rc;
...
static struct resource lcdc_geim_resources[] = {
	{
...
static void lcdc_geim_config_gpios(int enable)
{
	config_lcdc_gpio_table(lcdc_gpio_table,
		ARRAY_SIZE(lcdc_gpio_table), enable);
}

static struct msm_panel_common_pdata lcdc_geim_panel_data = {
	.panel_config_gpio = lcdc_geim_config_gpios,
	.gpio_num	  = lcdc_gpio_num,

};

static struct platform_device lcdc_geim_panel_device = {
#if defined(CONFIG_FB_MSM_LCDC_GEIM_HVGA)
	.name   = "lcdc_geim_hvga",
	.num_resources  = ARRAY_SIZE(lcdc_geim_resources),
	.resource       = lcdc_geim_resources,
#else
	.name   = "lcdc_s6d16a0x_hvga",
#endif
	.id     = 0,
	.dev    = {
		.platform_data = &lcdc_geim_panel_data,
	}
};

#if defined(CONFIG_KEYBOARD_STMPE1801) || defined(CONFIG_KEYBOARD_STMPE_IRQ)
static void stmpe1801_keyboard_reset(void)
{
	int rc = 0;
	printk("[STMPE1801] %s start \n", __func__);

	rc = gpio_request(84, "key reset");
	if(rc < 0) pr_err("failed to request key reset\n");

	gpio_tlmm_config(GPIO_CFG(83, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	//gpio_direction_input(83);

	gpio_tlmm_config(GPIO_CFG(84, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_direction_output(84, 1);
}
#endif

#ifdef CONFIG_TOUCHSCREEN_ZINITIX_GEIM
...
static int msm_fb_detect_panel(const char *name)
{
	int ret = -EPERM;
		...
		if (!strcmp(name, "lcdc_s6d16a0x_hvga"))
			...
			ret = -ENODEV;
#elif defined(CONFIG_FB_MSM_LCDC_GEIM_HVGA)
		if (!strcmp(name, "lcdc_geim_hvga"))
			ret = 0;
		else
			ret = -ENODEV;
#else
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	SND(AUDIENCE_HANDSET, 52),
	SND(AUDIENCE_HANDSET_LOOP, 53),
	SND(SAMSUNG_TTY_FULL, 54),
	SND(CURRENT, 0x7FFFFFFE),
};
#undef SND
#endif
...
static uint32_t camera_on_gpio_table[] = { // yjh

	GPIO_CFG(15, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), // mclk
	GPIO_CFG(96, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), // stby
	GPIO_CFG(85, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), // rst
...
static void keypad_gpio_init(void)
{
	...
	// Flip
	gpio_tlmm_config(GPIO_CFG(18, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
#else
...
// Flip
static struct gpio_event_direct_entry kp_flip_switch_map[] = {
	{ 18,	SW_LID	}
};
static struct gpio_event_input_info kp_flip_switch_info = {
	.info.func = gpio_event_input_func,
	.flags = 0,
	.type = EV_SW,
	.keymap = kp_flip_switch_map,
	.keymap_size = ARRAY_SIZE(kp_flip_switch_map)
};

static struct gpio_event_info *kp_info[] = {
	&kp_matrix_info,
	&kp_flip_switch_info.info,
};

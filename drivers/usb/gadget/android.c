...
static void get_usb_serial(char *usb_serial_number)
{
	...
	unique_serial_number = (system_serial_high << 16)
	...
	sprintf(temp_serial_number, "S7500%08x", unique_serial_number);
#elif defined(CONFIG_MACH_GEIM)
	sprintf(temp_serial_number, "I827-%08x", unique_serial_number);
#elif defined(CONFIG_MACH_JENA_NON_NFC)
	sprintf(temp_serial_number, "S6500D%08x", unique_serial_number);
#elif defined(CONFIG_MACH_JENA_TELSTRA) || defined(CONFIG_MACH_JENA_AUSVHA)
	sprintf(temp_serial_number, "S6500T%08x", unique_serial_number);
#elif defined(CONFIG_MACH_JENA)
	sprintf(temp_serial_number, "S6500%08x", unique_serial_number);
#else
...
void android_register_function(struct android_usb_function *f)
{
	...
	if (dev && should_bind_functions(dev)) {
		...
		/*Check LPM mode. LPM mode, the enumeration is not enable.*/
		if (dev->cdev && dev->cdev->gadget && !charging_boot)

...
#include "zinitix_touch_bt4x3_firmware_R005.h"
...
bool ts_init_touch(zinitix_touch_dev* touch_dev)
{
	...
	u16		MaxSupportedFingerNum = MAX_SUPPORTED_FINGER_NUM;
	u16		CurSupportedFingerNum;	
...
retry_init:
	...
	if (ts_read_data(touch_dev->client, ZINITIX_INT_ENABLE_FLAG, (u8*)&touch_dev->chip_int_mask, 2)<0) goto fail_init;

	if(touch_dev->chip_int_mask != 0)
	{
		printk(KERN_INFO "fail to disable chip interrupt(0x%x)\r\n", touch_dev->chip_int_mask);
		goto fail_init;
	}
	...
	curFirmware = (u16)chip_firmware_version;
	...
	if (ts_write_reg(touch_dev->client, ZINITIX_SUPPORTED_FINGER_NUM, (u16)MaxSupportedFingerNum)!=I2C_SUCCESS) goto fail_init;
...
static DEVICE_ATTR(tsp_firm_version_phone, 0664, firmware_phone_show, NULL);
static DEVICE_ATTR(tsp_firm_version_panel, 0664, firmware_panel_show, NULL);
static DEVICE_ATTR(tsp_firm_update, 0664, NULL, firmware_store);
...
//	gpio_set_value(78, 1);
#if (CONFIG_MACH_TREBON_HWREV == 0x0) || (CONFIG_MACH_GEIM_HWREV == 0x0)
	printk("[TSP] TSP POWER [ %s ]\n", gpio_get_value(41)?"on":"off");	
#else
	printk("[TSP] TSP POWER [ %s ]\n", gpio_get_value(78)?"on":"off");	
#endif
//	ts_power_enable(1);
	
//	disable_irq(client->irq);		//hash 0921
...
//	msleep(100);
//	ts_upgrade_firmware(touch_dev, &m_firmware_data[2], 32*1024);
//	msleep(100);
	
//	enable_irq(client->irq); //HASH 0921
	ts_init_touch(touch_dev);	

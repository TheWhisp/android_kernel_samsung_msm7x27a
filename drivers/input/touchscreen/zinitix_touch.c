...
#define	SYSTEM_MAX_X_RESOLUTION	320	//480
#define	SYSTEM_MAX_Y_RESOLUTION	480	//800
...
//#define GPIO_TOUCH_PIN_NUM	27//S5PV210_GPH0(3)	// interrupt pin number		HASH0721
//#define GPIO_TOUCH_IRQ  	MSM_GPIO_TO_INT( 27 )	// interrupt pin IRQ number	HASH0721
...
#define ZINITIX_DRIVER_NAME        "sec_touch"
...
u32 BUTTON_MAPPING_KEY[SUPPORTED_BUTTON_NUM] ={KEY_MENU, KEY_BACK }; //<= you must set key button mapping
...
static bool ts_get_samples (zinitix_touch_dev* touch_dev)
{
	...
	{
		if(ts_read_firmware_data(touch_dev->client,ZINITIX_RAWDATA_REG,(u8 *)touch_dev->cur_data, MAX_RAW_DATA*2)<0)
		{
		...
		{
			memcpy((u8*)touch_dev->ref_data, (u8*)touch_dev->cur_data, MAX_TEST_RAW_DATA*2);
		}
		up(&touch_dev->raw_data_lock); 
		return true;		
	}
...
static irqreturn_t ts_int_handler(int irq, void *dev)
{
	...
	if (gpio_get_value(27))	// remove pending interrupt
	{
...
bool ts_check_need_upgrade(u16 curVersion, u16 curRegVersion)
{
	...
	newFirmware = newVersion;	//HASH 0824
	curFirmware = curVersion;
...
#if BT4x3_Above_Series
	if(m_firmware_data[0x6412] == 0xff && m_firmware_data[0x6413] == 0xff)	return false;
	newVersion = (u16) (m_firmware_data[0x6412] | (m_firmware_data[0x6413]<<8));	
#endif	
...
#if 0//	TOUCH_USING_ISP_METHOD	
	if(strcmp(touch_dev->client->name, ZINITIX_ISP_NAME) == 0)
	{
		printk(KERN_INFO "isp client probe \r\n");
		m_isp_client = touch_dev->client;
		return 0;
	}
#endif	
//	m_isp_client = touch_dev->client;
...
#if	0//(TOUCH_USING_ISP_METHOD==0 && BT4x2_Series == 1)
	do{
...
//	ts_power_control(touch_dev, RESET_HIGH);	//reset pin high
	...
	for(flash_addr= 0; flash_addr< size; flash_addr+=TC_PAGE_SZ)
	{
		...
		i2c_buffer[0] = (flash_addr>>8)&0xff;	//addr_h
		i2c_buffer[1] = (flash_addr)&0xff;	//addr_l
#if 0	//BT4x3_Above_Series
		if(flash_addr == 0)	addr_buffer = (u8 *)i2c_buffer;
		else	addr_buffer = NULL;
		
		if(ts_read_firmware_data(m_isp_client, addr_buffer,&verify_data[flash_addr],TC_PAGE_SZ)<0)
#else
...
bool ts_init_touch(zinitix_touch_dev* touch_dev)
{
	...
	zinitix_debug_msg("touch reg data version = %d\r\n", chip_reg_data_version);
 	...
 	if(ts_check_need_upgrade(chip_firmware_version, chip_reg_data_version)==true)		//true	0812 HASH TEST 
   	{
...
//		touch_dev->work_proceedure = TS_NORMAL_WORK;
		...
		do
		{
			...
			if(touch_dev->work_proceedure != TS_NO_WORK)
			{
				...
				udelay(DELAY_FOR_SIGNAL_DELAY);
#endif				
				if (!gpio_get_value(27)) //touch_dev->int_gpio_num
				{
					ts_write_cmd(touch_dev->client, ZINITIX_CLEAR_INT_STATUS_CMD);
#if DELAY_FOR_SIGNAL_DELAY	
					udelay(DELAY_FOR_SIGNAL_DELAY);
	...
	if(touch_dev == NULL)
{
	...
	if(zinitix_bit_test(touch_dev->cur_data[MAX_TEST_RAW_DATA], BIT_PT_EXIST))
{
		buf[20] = (touch_dev->cur_data[MAX_TEST_RAW_DATA+1]&0xff);			//x_lsb
		buf[21] = ((touch_dev->cur_data[MAX_TEST_RAW_DATA+1]>>8)&0xff);		//x_msb
		buf[22] = (touch_dev->cur_data[MAX_TEST_RAW_DATA+2]&0xff);			//y_lsb
		buf[23] = ((touch_dev->cur_data[MAX_TEST_RAW_DATA+2]>>8)&0xff);		//y_msb
	}
	else	
	{
		buf[20] = 0; //x_lsb
...
static DEVICE_ATTR(tsp_firm_version_phone, 0660, firmware_phone_show, NULL);
static DEVICE_ATTR(tsp_firm_version_panel, 0660, firmware_panel_show, NULL);
		...
		case TOUCH_IOCTL_UPGRADE:
			{
				...
				disable_irq(misc_touch_dev->client->irq);//HASH??				
				down(&misc_touch_dev->work_proceedure_lock);	
				misc_touch_dev->work_proceedure = IN_UPGRADE;
#if	ZINITIX_ESD_TIMER_INTERVAL	
				...
				if(misc_touch_dev->use_esd_timer)
				{
					ts_esd_timer_start(ZINITIX_ESD_TIMER_INTERVAL*2, misc_touch_dev);
					zinitix_debug_msg("esd timer start\r\n");
				}
#endif				
				enable_irq(misc_touch_dev->client->irq);//HASH??
				misc_touch_dev->work_proceedure = NO_WORK;
...
//	touch_dev->int_gpio_num = GPIO_TOUCH_PIN_NUM;	// for upgrade	//HASH0714
...
//	zinitix_debug_msg("request irq (irq = %d, pin = %d) \r\n", touch_dev->irq, touch_dev->int_gpio_num);
	...
	if (client->irq) {
		ret = request_irq(client->irq, ts_int_handler, IRQF_TRIGGER_LOW , ZINITIX_DRIVER_NAME, touch_dev);
		if (ret) {
	...
	if (device_create_file(&client->dev, &dev_attr_get_touch_test_raw_data) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_get_touch_test_raw_data.attr.name);
	if (device_create_file(&client->dev, &dev_attr_set_touch_testmode) < 0)
...
//		pr_err("Failed to create class(sec)!\n");
	...
	if (ret) {
		sain_debug("Fail to register touch misc device.\n");
	}
#endif

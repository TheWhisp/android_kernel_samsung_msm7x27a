...
**/
	...
	if (touch_dev->touch_info.status == 0x9)
		...
		zinitix_debug_msg("R [00] x = %03d, y = %d\r\n", x, y);
#else
		printk(KERN_INFO "[TSP] R [00] \r\n");
#endif
...
#else
				printk(KERN_INFO "[TSP] button down \r\n");
#endif
...
#else
				printk(KERN_INFO "[TSP] button up \r\n");
#endif
				if (BUTTON_MAPPING_KEY[i] == KEY_MENU)
					menu_pressed = 0;
				else if (BUTTON_MAPPING_KEY[i] == KEY_BACK)
...
/**			zinitix_debug_msg("finger [%02d] x = %d, y = %d \r\n",
				i, x, y); */
			if (y >= SYSTEM_MAX_Y_RESOLUTION)
				goto continue_read_samples;
			if (touch_dev->touch_info.status == 0x902)
#ifdef DEBUG_TOUCHSCREEN_ZINITIX
				zinitix_debug_msg("P [%02d] x = %03d, "
							"y = %d\r\n", i, x, y);
#else
	...
	struct device_attribute *attr, char *buf)
{
	...
	int16_t diff_data[14][8] = {{0,},};
	int16_t raw_data[14][8] = {{0,},};
	...
	for (i = 0; i < 14; i++) {
		for (j = 0; j < 8; j++) {
			...
			diff_data[i][j] = (int)(((s16)(g_touch_dev->cur_data[k]-
					g_touch_dev->ref_data[k]))&0xffff);
			k++;
	}
	}
	...
	for (i = 0; i < 14; i++) {
		printk(KERN_CONT "[TSP]");
		for (j = 0 ; j < 8 ; j++)
			printk(KERN_CONT " %5d", diff_data[i][j]);
		printk(KERN_CONT "\n");
	}
	printk(KERN_CONT "\n");
	...
	for (i = 0; i < 14; i++) {
		for (j = 0; j < 8; j++) {
			snprintf(buf+written_bytes, 7, "%d\n", diff_data[i][j]);
			written_bytes += 6;
		}
	}
...
static ssize_t get_tsp_threshold(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u16	ts_threshold;
	if (ts_read_data(g_touch_dev->client, 0x20, (u8 *)&ts_threshold, 2) < 0)
			return snprintf(buf, 24, "ts_read_data FAILED!!\n");

	return snprintf(buf, sizeof(int), "%d\n", ts_threshold);
}

/*S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, S_IRUGO | S_IWUSR | S_IWGRP */
static DEVICE_ATTR(get_touch_raw_data,
	S_IRUGO | S_IWUSR | S_IWGRP,
	zinitix_get_test_raw_data,
	zinitix_set_testmode);
static DEVICE_ATTR(get_threshold, S_IRUGO, get_tsp_threshold, NULL);
		...
		struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_DEBUG "tkey_rawcounter : search=%d\n",
			tkey_rawdata);
...
*******/
	...
	if (device_create_file(sec_touchscreen_dev,
		&dev_attr_get_threshold) < 0)
		pr_err("Failed to create device file(%s)!\n",
			dev_attr_get_threshold.attr.name);

...
static void tsp_vreg_config(int vreg_en)
{
	int rc;
...
//todo0517	vreg_touch = vreg_get(NULL, "ldo11");
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[TSP] disable_irq, ln: %d\n", __LINE__);
	disable_irq(client->irq);

	printk("[TSP] %s, %d\n", __func__, __LINE__ );

	tsp_vreg_config(VREG_ENABLE);
...
//	printk("[TSP] %s set SCL high, %d\n", __func__, __LINE__ );
	...
	printk("[TSP] %s ====== before INIT_WORK(synaptics_ts_work_func), %d\n", __func__, __LINE__ );
...
//	ts->timer.function = cypress_watchdog_timer_func;
	...
	*/
	...
	printk(KERN_INFO "[TSP] synaptics_ts_probe: Manufacturer ID: %x, HW ver=%d\n", buf[0], HW_ver);
...
//		return -ENOMEM;
	...
	printk("[TSP] %s  before i2c_add_driver!\n", __func__ );
	return i2c_add_driver(&synaptics_ts_driver);
	printk("[TSP] %s  after i2c_add_driver!\n", __func__ );
}
